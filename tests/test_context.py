...
class TestContextRegisters:
    ...
    def test_cannot_update_context_register_from_lists(self):
        left = Statement(
            "$shooter shot $victim", terms=[Term("Alice"), Term("Bob")]
        )
        right = Statement(
            "$shooter shot $victim", terms=[Term("Craig"), Term("Dan")]
        )
...
class TestChangeRegisters:
    def test_reverse_key_and_value_of_register(self):
        ...
        assert register.get("<Siskel>") == Term("Ebert")
    ...
    def test_factor_pairs(self):
        ...
        assert next(gen) == (Term("apple"), Term("pear"))
